...
fn main() {
    // `hearth-gpui-assets` exposes the absolute path of its default
    // icons directory via cargo's `links` mechanism (see its `Cargo.toml`
    // and `build.rs`). We receive that path as
    // `DEP_HEARTH_GPUI_DEFAULT_ICONS_ICONS_DIR` here, then re-publish
    // it as a `rustc-env` so it's visible to the
    // `icon_named!("$HEARTH_GPUI_DEFAULT_ICONS_DIR")` proc-macro call
    ...
    // (not `build-dependencies`) entry for `hearth-gpui-assets` in
    // `Cargo.toml`.
    let icons_dir = env::var("DEP_HEARTH_GPUI_DEFAULT_ICONS_ICONS_DIR").expect(
        "DEP_HEARTH_GPUI_DEFAULT_ICONS_ICONS_DIR is set by hearth-gpui-assets's \
         build.rs via its `links` field; make sure the regular dependency on \
         hearth-gpui-assets is intact in Cargo.toml",
    );

    println!("cargo:rustc-env=HEARTH_GPUI_DEFAULT_ICONS_DIR={icons_dir}");
